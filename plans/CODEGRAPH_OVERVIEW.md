...
Gradio 4.0+ 应用：将 Excel 模板（如 Ginger Lots）可视化为 Web 表单，支持多区域检测、YAML 驱动批量导入、Google Sheet 按 ID 查询填表、Phi-4 智能字段匹配，并导出/打印 xlsx。每个模板通过 `templates/` 自动发现，配置分散在模板子目录 sidecar 文件中。

**LLM 约束：** Phi-4-mini-instruct 约 **4B** 参数、**GGUF Q4_K_M**、**仅 CPU 推理**（无 GPU 依赖与性能优化路线）。
...
| `app/components/gradio_config.py` | 「参数配置」Tab：区域配置、YAML 编辑、Phi-4 LLM 测试与自动配置（**批量匹配**） |
...
| `app/services/phi4_field_matcher.py` | Phi-4-mini GGUF 字段匹配（Transformers + gguf，本地 CPU；含 `batch_match_all_fields`） |
...
| `scripts/download_phi4_model.py` | 下载 Phi-4 GGUF 至 `models/phi4/` |
| `templates/{id}/` | 模板 sidecar：`.paste.yaml`、`.datasource.json`、`.history.json` |
| `models/phi4/` | Phi-4 GGUF 权重（不入库） |
...
| `plans/batch_llm_matching/` | 批量 LLM 匹配 Speckit（已实现于 `gradio_config` / `phi4_field_matcher`） |
...
**已删除遗留 Streamlit / Phi-3.5 模块（2026-06-14）：** `app/main.py`、`template_form.py`、`data_source_settings.py`、`paste_parse_settings.py`、`paste_image_button.py`（含 frontend）、`phi35_vision_model.py`、`phi35_vision_paste_infer.py`、`source_parser.py`、`paste_mapping_infer.py`、`shutdown.py`。
...
| 参数配置 | `gradio_config.build_config_tab` | 子 Tab：区域配置 / YAML 编辑 / Phi-4 LLM 字段匹配测试与「自动配置」（批量 Prompt） |
...
| 维度 | 实现 |
|------|------|
| 批量推理 | `Phi4FieldMatcher.batch_match_all_fields` — 单次 Prompt 映射全部 YAML 字段 |
...
4. **批量导入：** `fetch_all_rows` → Phi-4 字段匹配 → 预览 → 选中导入 → `import_history`。
...
| `phi4_field_matcher` | `get_or_create_field_matcher` | 单例匹配器（含加载进度） |
| `phi4_field_matcher` | `ensure_model_downloaded` | 自动下载 GGUF |
| `phi4_field_matcher` | `batch_match_all_fields` | 单次 LLM 批量字段映射 |
| `phi4_field_matcher` | `iter_match_sheet_fields_to_yaml` | 逐字段匹配（批量导入） |
...
| CodeGraph CLI 实体 | 227（`plans/codegraph.csv`，2026-06-14 清理后） |
| 活跃 Speckit 计划 | `gradio_ui_migration`、`llm_field_matching_optimization`、`batch_llm_matching` |
| LLM | Phi-4-mini-instruct ~4B，GGUF Q4_K_M，CPU only |
| 外部依赖 | gradio, pandas, polars, openpyxl, gspread, PyYAML, torch, transformers, gguf |
