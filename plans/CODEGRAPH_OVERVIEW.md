...
**LLM 约束：** `google/gemma-4-E4B-it-qat-q4_0-gguf` · **E4B Q4_0 GGUF** · **Transformers 5.x + GGUF（纯 Python，无 llama-cpp）** · **CPU 推理** · 可用内存 ≥5 GB · 权重在 `models/gemma4/gemma-4-E4B_q4_0-it.gguf`。
...
| `app/services/gemma4_field_matcher.py` | Gemma 4 GGUF 字段匹配（Transformers 5.x，本地 CPU；含 `batch_match_all_fields`） |
...
| `models/gemma4/` | Gemma 4 GGUF 权重（不入库） |
...
**已删除遗留 Streamlit / Phi-3.5 / Phi-4 / llama-cpp / mobile-transformers 模块（2026-06-14）：** `app/main.py`、`template_form.py`、`data_source_settings.py`、`paste_parse_settings.py`、`paste_image_button.py`（含 frontend）、`phi35_vision_model.py`、`phi35_vision_paste_infer.py`、`source_parser.py`、`paste_mapping_infer.py`、`shutdown.py`、`app/services/phi4_field_matcher.py`、`scripts/download_phi4_model.py`、`tests/test_llm_improvements.py`。
...
**依赖：** `requirements.txt` — gradio, pandas, polars, openpyxl, gspread, PyYAML, torch, transformers>=5.0, huggingface-hub, psutil, google-auth-oauthlib。
...
| 维度 | 实现 |
|------|------|
| 模型 | `google/gemma-4-E4B-it-qat-q4_0-gguf` → `models/gemma4/gemma-4-E4B_q4_0-it.gguf` |
| 加载 | `AutoTokenizer` + `AutoModelForCausalLM.from_pretrained(gguf_path)`（Transformers 5.x） |
| 内存门控 | `get_available_memory_gb()` < 5 GB → `InsufficientMemoryError` |
| 批量推理 | `Gemma4FieldMatcher.batch_match_all_fields` — 单次 Prompt 映射全部 YAML 字段 |
| 调用方 | `gradio_config.prepare_llm_test_prompt` + `run_llm_test_generation`、`handle_yaml_auto_config` |
| 逐字段回退 | `iter_match_sheet_fields_to_yaml` / `_llm_match_column`（批量导入 Tab 仍用） |
| 样本行 | ≥5 行，`prepare_batch_input` → `SourceColumnData` |
| Prompt | `build_batch_field_mapping_prompt` |
...
7. **LLM 测试：** 选定 `test_cols` → 读 Sheet 样本 → **批量匹配**展示 Prompt/响应（`prepare_llm_test_prompt` → `run_llm_test_generation`）。
...
| `gemma4_field_matcher` | `ensure_model_downloaded` | 自动下载 GGUF |
...
| `gradio_config` | `prepare_llm_test_prompt` / `run_llm_test_generation` | LLM 测试（Prompt/响应/YAML JSON） |
