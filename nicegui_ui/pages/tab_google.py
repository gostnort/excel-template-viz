...
def _id_in_database(session, row_id: str) -> bool:
    ...
    if not use_db:
        ...
        pk_label = next(
            (
                rule.Input_label
                for rule in getattr(session.cfg, "field_rules", [])
        ...
        with ui.element("div").classes("section"):
            ...
            with ui.element("div").classes("section-body"):
                ...
                upload = (
                    ...
                    .props('accept=".json" no-thumbnails auto-hide-upload-progress')
                    .style(
                ...
                with ui.element("div").classes("sheet-toolbar-bar"):
                    with ui.element("div").classes("toolbar-main"):
                        ...
                        def handle_import() -> None:
                            ...
                            """
                            if (
                                not getattr(session, "google_op", None)
                                or not session.google_connected
                            ):
                            ...
                            try:
                                ...
                                ui.run_javascript(
                                    "Array.from(document.querySelectorAll('.tabs .tab')).find(el => el.textContent === '输入')?.click();"
                                )
