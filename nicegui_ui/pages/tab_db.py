        ...
        with ui.element('div').classes('section'):
            ...
            with ui.element('div').classes('section-body'):
                ...
                if not data:
                    ...
                    with ui.element('div').classes('overflow-y-auto max-h-[500px] border border-gray-300 w-full mt-2'):
                        ...
                        with ui.element('tbody'):
                            ...
                            for row in displayed_data:
                                ...
                                with ui.element('tr').classes('cursor-pointer').on(
                                    'click',
                                    lambda _e=None, rid=row_id: _select_db_row(session, rid, session.use_independent_db),
                                ):
                                    if not session.use_independent_db:
                                        with ui.element('td'):
                                            ui.label(str(row_id) if row_id is not None else '')
                                    for lbl in labels:
                                        with ui.element('td'):
                    ...
                    if not session.use_independent_db and getattr(session, 'loaded_offset_k', 0) > 0:
                        ...
                        with ui.row().classes('justify-center w-full my-2'):
                            ui.button(f'加载更多 (剩余 {session.loaded_offset_k} 行)', on_click=load_next_batch_excel).props('flat dense')
                    elif session.use_independent_db and len(data) > getattr(session, 'db_loaded_limit', 50):
                        def load_next_batch_db():
                            session.db_loaded_limit = getattr(session, 'db_loaded_limit', 50) + 50
                            render_db_tab.refresh()
                        with ui.row().classes('justify-center w-full my-2'):
                            ui.button('加载更多 (50行)', on_click=load_next_batch_db).props('flat dense')
                ...
                def on_overwrite(e):
                    ...
                    try:
                        ...
                        if session.use_independent_db:
                            ...
                            session.writer.write_back(session.template_path, session.template_path, merged, session.selected_instance_k)
                            from nicegui_ui.components.for_main import ForMain
                            ForMain.load_template(session.template_id, str(session.template_path))
