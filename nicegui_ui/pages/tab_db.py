...
from app.services.core_store import list_db_paths, allocate_next_db_path
        ...
        with ui.element('div').classes('section'):
            ...
            with ui.element('div').classes('section-body'):
                with ui.element('div').classes('row'):
                    ...
                    db_select = ui.select(options, value=current_db_str, label='').classes('dropdown narrow').props('dense borderless')
                    ...
                    def on_switch():
                        ...
                        from app.services.core_store import SecureSQLite, UiProvider
                        session.db.close()
                    ...
                    ui.label('切换').classes('btn').on('click', on_switch).bind_classes_from(db_select, 'value', backward=lambda v: 'btn' if v != current_db_str else 'btn disabled')
                    ...
                    ui.label('新建库').classes('btn').on('click', on_new_db)
        ...
        with ui.element('div').classes('section'):
            ...
            with ui.element('div').classes('section-body'):
                ...
                if not data:
                    ...
                    if not pk_label and labels:
                        pk_label = labels[0]
                        
                    def on_table_select(e):
                        if e.selection:
                            session.selected_db_row_index = e.selection[0].get(pk_label)
                        else:
                            session.selected_db_row_index = None
                            
                    table = ui.table(columns=columns, rows=data, row_key=pk_label, selection='single').classes('w-full').style('border-radius: 0; box-shadow: none; border: 1px solid #000;')
        ...
        with ui.element('div').classes('section'):
            ...
            with ui.element('div').classes('section-body'):
                ...
                with ui.element('div').classes('row').style('margin-top:8px'):
                    ui.label('覆盖保存').classes('btn').on('click', on_overwrite)
