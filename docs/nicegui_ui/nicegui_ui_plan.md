...
> 修订 2026-07-18：Shell 改为 **CSS Grid**（`grid-template-columns: auto 1fr`）；侧栏宽度由内容撑开，**不再**使用 `ui.splitter` / `sidebar_width`；线框与 `nicegui_ui_*.html` 同步。
> 修订 2026-07-18（早）：`use_independent_db` 落盘至模板 TOML；输入 Tab 工具栏与两步删除对齐实现。
...
Wireframes in `docs/nicegui_ui/nicegui_ui_*.html` are **layout references** for spacing and control grouping. Runtime uses NiceGUI / Quasar primitives (`ui.element` grid shell, `ui.textarea`, `ui.checkbox`, HTML `<table>`) — wireframe class names (e.g. `.shell`, `.btn`) map to CSS + label/button patterns, not necessarily identical DOM.
...
nicegui_ui/
  ...
  pages/
    main.py              # CSS Grid shell; wires sidebar + tab panels
    sidebar.py
    tab_input.py
    tab_google.py
    tab_toml.py
...
## 2. Layout Parity & Grid Shell Constraints

The shell must match `docs/nicegui_ui/nicegui_ui_index.html` (2026-07 wireframe refresh): **CSS Grid** with four direct children — `sidebar-header`, `tabs`, `sidebar`, `main`. The layout must occupy the **entire viewport with zero outer margins or paddings**. A single-column unstyled page is unacceptable.

```
+-----------------------------------------------------------------------------+
| .shell  (display: grid; grid-template-columns: auto 1fr)                  |
|  row1  [ template_id  << ] | [ 输入 ][ 输入配置 ][ 存储配置 ][ Google 连接 ] |
|  row2  | Template list    | Tab body (scroll)                              |
|        | (.sidebar)       | (.main)                                        |
+--------+--------------------------------------------------------------------+
```
...
* **Grid areas:** `sidebar-header` → column 1 row 1; `tabs` → column 2 row 1; `sidebar` → column 1 row 2; `main` → column 2 row 2.
* **Left column width:** `grid-template-columns: auto 1fr` — the left column sizes to the **wider of** `sidebar-header` (template name + fold chevron) and the template list. **No** fixed pixel width, **no** `ui.splitter`, **no** drag resize rail.
* **`sidebar-header`:** selected `template_id` / display name + fold chevron `<<` / `>>` on the right. `white-space: nowrap` so the header defines minimum left-column width when names are long.
* **Collapse:** `.shell.is-sidebar-collapsed` hides **only** `.sidebar` (template list). Header and tabs stay visible. Persist `sidebar_collapsed` (bool) in `app.storage.user`; **do not** persist `sidebar_width`.
* **Template list:** no horizontal divider lines between items (wireframe `.template-item` has padding only).
* **Tabs:** fixed width **126px** each; active tab uses negative `margin-bottom` + matching `border-bottom` color so the tab **merges** with the content below (no thick line under the active tab).
...
* **Presentation:** vertical list in `.sidebar` (grid row 2, column 1), not a dropdown. Data from `SortTemplates.UpdateJson()` → `TemplateIDs`, `template_display_names`, `sort_timeline`.
* **Interaction:** click item → run template activation → highlight active row → update **top-bar selected template name** in `sidebar-header`.
* **Header:** show `{template_display_name}` or `{template_id}` when active; `未选择` otherwise.
...
* **NiceGUI approach:** `render_shell()` in `main.py` emits four sibling elements under `.shell`; template list via `@ui.refreshable` in `sidebar.py`.

### 2.2 Sidebar Width & Collapse (auto column — no splitter)

**Runtime (`nicegui_ui/components/style.css`):**
...
.sidebar-header { grid-column: 1; grid-row: 1; white-space: nowrap; }
.tabs           { grid-column: 2; grid-row: 1; }
.sidebar        { grid-column: 1; grid-row: 2; }
.main           { grid-column: 2; grid-row: 2; min-height: 0; overflow: hidden; }
.shell.is-sidebar-collapsed .sidebar { display: none; }
```

**Persisted preferences (`app.storage.user` + `Auth.pref_key`):**

| Key | Type | Purpose |
|-----|------|---------|
| `sidebar_collapsed` | bool | Fold button only |
| `active_tab` | str | Last selected tab label |

**Removed / forbidden:**

* `sidebar_width` — no longer read or written.
* `ui.splitter`, `limits=(0,1000)`, resize callbacks, `.resize-rail` wireframe stand-in.
* Setting splitter `value = 0` to collapse.

**Fold button (`.sidebar-fold-btn`):**

* Toggle `sidebar_collapsed` + `.is-sidebar-collapsed` on `.shell`.
* Chevron `aria-expanded` reflects state; list hidden when collapsed, header name always visible.

**Why Grid instead of splitter:** long `template_id` strings must widen the left column together with the header; JS sync between splitter and header was fragile. `auto` column tracks content naturally.

### 2.3 Right Area: Tabs Layout & Spacing
> **CSS 约定（禁止全局冲突）**：业务横向工具行使用 `.form-row`，**禁止**在 `style.css` 中定义全局 `.row`，以免与 Quasar 的 `ui.row` 或 grid 布局冲突。
>
> **`!important` 政策**：不要随便在 CSS 中使用 `!important`。应先定位冲突根因并修复；`!important` 仅作最后手段，且须用户明确同意。

* **Tab width:** each tab **126px** (`width` / `min-width` / `max-width`).
* **Active tab:** `margin-bottom: -2px; border-bottom: 2px solid #eee` (matches tab background) so the tab bar bottom rule does not show through the active tab.
* **Scrolling:** `.main` and active tab panels use `overflow-y: auto`; wide tables scroll inside `.sheet-table-scroll` or `min-width: 0` flex children.
* **Input vertical alignment:** single-line field text vertically centered in the control box (Quasar `.q-field__inner` flex); ghost textarea remains top-aligned for multiline paste.
...
* **NiceGUI:** `ui.tabs()` in grid row 1 col 2; `ui.tab_panels()` inside `.main` row 2 col 2. Tab labels: `输入`, `输入配置`, `存储配置`, `Google 连接`.
...
* **Shell:** keep `sidebar-header` + `tabs`. On narrow screens default **sidebar collapsed**; fold chevron is the primary navigation back to template list.
...
|------|-------|------------|------------------|
| `nicegui_ui_index.html` | CSS Grid + fold | 输入 | `.field-grid`; toolbar 保存/刷新/删除 ‖ 添加数据；delete_mode 勾选列 |
| `nicegui_ui_toml.html` | same | 输入配置 | 校验并应用 + TOML 高级（含 `use_independent_db` 占位；无 AI 向导） |
| `nicegui_ui_db.html` | same | 存储配置 | §3.4 checkbox → `cfg.Save` / TOML `use_independent_db` |
| `nicegui_ui_connect.html` | same | Google 连接 | OAuth row + 主 ID 表（`.sheet-table-scroll`）+ 屏蔽所选数据 |

Wireframes use **placeholder** labels only (显示名甲、字段A、列1…) — black-and-white layout reference, not production data. Sub-tab wireframes may include fold-toggle demo JS; layout classes must match `index`.
...
* **Main ID sheet table:** `ui.table` with multi-select inside `.sheet-table-scroll` (horizontal overflow); `全选` / `取消全选` / `导入选中行`.
...
Wireframe: `nicegui_ui_toml.html`. Same CSS Grid shell as index.
...
Wireframe: `nicegui_ui_db.html`. Same CSS Grid shell as index.
...
| Sidebar collapsed / active tab | `app.storage.user` (`Auth.pref_key`) | survives reload; needs `storage_secret` |
...
|-------------|----------------------|------------------------|
| App shell | `gr.Row` + CSS overrides | CSS Grid `.shell` (auto + 1fr); fold chevron in `sidebar-header` |
| Sidebar list | `gr.HTML` + JS click bridge | `ui.list` / `ui.button` + Python `on_click` |
| Resize / collapse | custom `#resize-rail` + `localStorage` | CSS Grid auto column + `.is-sidebar-collapsed` + `app.storage.user` |
...
1. **Spike shell:** CSS Grid `.shell` (template name + fold + tabs + sidebar + main); sidebar collapse persistence.
...
2. CSS Grid shell: left column `auto` (header + list same width); no `sidebar_width`; fold toggles `.is-sidebar-collapsed`; tabs 126px; active tab no bottom rule.
