...
采用 **TOML 1.0 严格范式**：`tomlkit` 写入，`tomllib` 读取。文件中**不使用** `null`（TOML 标准无此类型）。
...
|------|------|
| 内存 / API | Python `None` |
| 磁盘文件 | **省略**可选键，不写 `null`、不写空字符串 |

`[[fields]]` 每一行：

- **必有键**：`Input_label`、`index`、`id`
- **可选键**：`filed`、`source_file`、`source_sheet`、`regex`
- 可选键缺失或读入为 `""` → 应用层视为 `None`
- `index = -1` 表示无文本框列索引（整数键保留）

`[[sources]]`：路径尚未配置时**整段省略**；有路径后用 `tomlkit.string(..., literal=True)` 写单引号字面量。
...
# 首次生成时不写 [[sources]]，真实路径由专用配置页写入后再落盘
[[sources]]
source1 = 'c:\temp\cache\执法堂业绩.xlsx'
source2 = 'c:\temp\cache\执法堂业绩.xlsx'
...
index = -1            # 无文本框列索引；无 filed / source_* / regex
id = false
...
4. **已映射但无 regex**：省略 `regex` 键。
5. **默认空配置**：仅 `determiner`、`worksheet`（若有）、`sections`（若有）、`[[fields]]` 骨架；无 `[[sources]]`。
...
`tomllib` 为 Python 3.11+ 标准库，只读；写入用 `tomlkit.dumps`。
