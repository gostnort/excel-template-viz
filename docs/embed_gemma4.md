...
> 状态：v6.8（**仅** 推理驱动、结构化判定接口与内容交互；不含向导编排、不含 UI、**不含** 各业务域 prompt）  
> 日期：2026-07-12  
...
```
llm_gemma4/
  __main__.py              # ConversationOnce/StartGemma/EndGemma + Pic2Str（四者共用同一单例，见§3.1d）；`python -m llm_gemma4 "问题"` 可直接跑
  config.py
  models_catalog.py
  hf_download.py
  profiles/
  ...
  backends/
    ...
    factory.py
    litert/
      backend.py           # LiteRtBackend：Engine 单例 + generate/generate_vision（临时 Conversation）
...
内部持有**一个模块级单例 backend**（首次调用时 `create_backend()`，之后复用），不是每次调用都新建——这是 §1.1 GPU 成本回收实测的直接结论：只要连续调用 ≥2 次，复用 `Engine` 就已经比重新构建划算。`ResetBackend()` 用于测试/进程退出时显式关闭。**不**做判定/解析/规范化——需要稳定三态判定的场景仍用 §3.6 `run_judgment`，`ConversationOnce` 只是 `generate()` 的薄封装，返回 `GenerateResult.text`。
...
#### 3.1c `StartGemma`/`EndGemma`：显式控制常驻的起止（2026-07-11 新增）

给"长生命周期进程"（见上段）用的显式生命周期对：

```python
# llm_gemma4/__main__.py
def StartGemma() -> None: ...   # 主动加载并常驻，按硬件探测选 NPU/GPU/CPU
def EndGemma() -> None: ...     # 卸载 StartGemma()（或任意一次 ConversationOnce()）加载的模型
```

`StartGemma()`/`EndGemma()` 跟 `ConversationOnce()`/`Pic2Str()` 共用**同一个**模块级单例 `_backend`（2026-07-12 起合并，见 §3.1d 末段）——`ConversationOnce()` 原有的"没加载就现加载"逻辑完全不用改：如果调用方已经先调过 `StartGemma()`，`ConversationOnce()`/`Pic2Str()` 内部的 `_get_backend()` 直接拿到现成的、已加载好的单例，不会重复触发一次加载。调用顺序不影响正确性，只影响冷启动成本记在哪一次调用头上。

**实现坑（已实测踩过、已修）**：`_get_backend()` 里的 `create_backend()` 只构造一个轻量的 `LiteRtBackend` **包装对象**（`__init__` 只是存 profile、把 `self._engine` 置 `None`），真正重的 `Engine` 构造是惰性的，原本要等第一次 `generate()` 内部调 `_ensure_engine()` 才会触发。`StartGemma()` 如果只调 `_get_backend()` 是不够的——实测过：那样 `StartGemma()` 本身几乎瞬间返回（<0.1s），冷启动成本原样转嫁到了下一次 `ConversationOnce()`（仍然要等 10+ 秒），完全没有起到"提前扛冷启动"的效果。修法是给 `LlmBackend` 协议加一个新方法 `warm() -> None`（`backends/base.py`），`LiteRtBackend.warm()` 直接调 `self._ensure_engine()`；`StartGemma()` 改成调 `_get_backend().warm()`。已用真实 `litert_lm` + `gemma-4-E4B-it.litertlm` 实测验证：加 `warm()` 后 `StartGemma()` 本身吃满 ~11s 冷启动，随后的 `ConversationOnce()` 明显变快。

`EndGemma()` 是 `ResetBackend()` 的对外别名（同一份实现），只是名字跟 `StartGemma` 成对，避免调用方要去记一个偏内部测试向的 `ResetBackend` 名字。

#### 3.1d `Pic2Str`：通用图片→文本接口（2026-07-12 新增；2026-07-12 合并单例）

```python
# llm_gemma4/__main__.py
def Pic2Str(image: str | Path | bytes, prompt: str, *, system=None, max_tokens=None, temperature=0.0) -> str: ...
```

**没有独立的 `StartGemmaVision`/`EndGemmaVision`**：单例已合并（见下），`Pic2Str` 跟 `ConversationOnce` 共用同一个 `_backend`，预热/卸载统一用 `StartGemma()`/`EndGemma()`。开发阶段不为"看起来语义对称"保留一层纯别名——语义清晰、结构简单优先于向后兼容（这两个别名此前存在过，2026-07-12 当天就因为这条原则被去掉了）。

**实测已核实（2026-07-12，真实 `litert_lm` 0.14.0 + `gemma-4-E4B-it.litertlm`，CPU）**：`litert_lm.Content` 有 `ImageFile(absolute_path)` / `ImageBytes(bytes)`（后者要已编码的图片字节，如 jpg/png 原始字节，不是解码后的像素数组），`Engine(model_path, backend=..., vision_backend=...)` 多一个 `vision_backend` 构造参数；`litert_lm.Contents.of(image_content, prompt_text)` 拼成一条多模态 user 消息喂给 `Conversation.send_message`。已用真实图片验证：`Engine(vision_backend=Backend.CPU())` 构造不报错，`send_message` 真的会读图片内容（不是忽略图片纯靠文本瞎猜）。

**关键：我们下载的这个具体权重文件是支持视觉的**，同一个 HF 仓库 `litert-community/gemma-4-E4B-it-litert-lm` 下有两个文件——`gemma-4-E4B-it-web.litertlm`（3.0GB，官方讨论区确认 **web 转换目前是纯文本**，喂图片会在 C++ 层报 `LlmVisionInferenceCalculator failed`）和我们用的 `gemma-4-E4B-it.litertlm`（3.7GB，桌面/CLI 版，多出来的体积对应视觉塔权重，实测确认能读图）。**下次升级/更换模型文件时要重新确认这一点**，不能想当然认为"同一个模型家族名字"就一定带视觉。

**单例已合并（2026-07-12）**：上线时最初的设计是给 `Pic2Str` 配一个独立于 `ConversationOnce` 的第二个模块级单例 `_vision_backend`（`create_backend(enable_vision=True)`），配套一对 `StartGemmaVision()`/`EndGemmaVision()`，理由是 `vision_backend` 是 `Engine` **构造时**才能定的参数，已经构造好的纯文本 Engine 不能事后"追加"视觉能力，所以当时假设视觉塔会给不需要看图的纯文本调用方（`run_judgment`、TOML 向导等）强加一块显存/内存开销。**实测推翻了这个假设**：用 `test/llm_gemma4/probe_vision_backend_text_judgment.py` 拉起带 `vision_backend` 的 Engine 跑纯文本判定，肉眼盯显存对比——带视觉的 Engine 显存占用 ~4.0GB，跟纯文本 Engine 的 ~3.6GB 几乎没差别（此前记录的 4.6GB 峰值里有 ~1GB 是别的常驻占用，不是视觉塔本身的增量）。既然差距可忽略，就不值得为了省这不到 0.5GB 而维护两份 Engine（两份意味着任何同时需要判定+识图的调用方都会同时常驻两个 ~4GB 的 Engine）。于是合并为**一个**模块级单例 `_backend`：`_get_backend()` 现在恒以 `create_backend(enable_vision=True)` 构造，`ConversationOnce`/`Pic2Str` 共用它；`StartGemma()`/`EndGemma()` 一次预热/卸载两者都受益。原来的 `StartGemmaVision()`/`EndGemmaVision()`/`ResetVisionBackend()` 已**直接删除**（不留别名）——开发阶段没有外部调用方需要兼容，语义清晰、结构简单优先于保留历史命名。

**本函数不含任何业务语义**：`Pic2Str` 只保证接口通、原样透传模型回答的字符串（哪怕 prompt 里要求 JSON/TOML 格式化输出也不解析/不校验）；识别精度是否够用、要不要拿它替代/辅助某条现有 OCR 流程，都是调用方（如 `paddle_ocr`）的业务判断，不属于本平台职责（同 §0.1 解耦原则）。**已知局限（实测记录，非本函数的 bug）**：拿真实机场遗失物品单据的样图测试过，无论是喂一条细长裁剪条还是喂整页图，模型都读不准密集小字中文——不是完全不读图（两次给的答案不同、且都不是纯文本纠错原来的答案），但也不是可信的逐字 OCR，会编出通顺但跟图片实际内容不符的文字。Gemma4 的视觉塔是通用图像理解（版式/粗粒度语义），不是针对文档 OCR 精调过的模型，这点跟专门做文档 OCR 的 PaddleOCR-VL 不是一个量级，调用方不应假设 `Pic2Str` 能达到专用 OCR 模型的精度。
...
class LlmBackend(Protocol):
    ...
    def warm(self) -> None: ...
    """把惰性的重构造（比如真正的 Engine）提前逼出来，不等第一次 generate()/
    open_session()。2026-07-11 为 §3.1c StartGemma() 新增；没有惰性构造的
    backend 可以让这个方法什么都不做。"""
