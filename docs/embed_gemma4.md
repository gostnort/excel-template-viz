...
```
llm_gemma4/
  __main__.py              # ConversationOnce/StartGemma/EndGemma；`python -m llm_gemma4 "问题"` 可直接跑
  config.py
  models_catalog.py
  hf_download.py
  profiles/
...
`StartGemma()`/`EndGemma()` 跟 `ConversationOnce()` 共用**同一个**模块级单例 `_backend`——`ConversationOnce()` 原有的"没加载就现加载"逻辑完全不用改：如果调用方已经先调过 `StartGemma()`，`ConversationOnce()` 内部的 `_get_backend()` 直接拿到现成的、已加载好的单例，不会重复触发一次加载。调用顺序不影响正确性，只影响冷启动成本记在哪一次调用头上。
