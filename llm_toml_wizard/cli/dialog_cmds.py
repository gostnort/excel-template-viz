...
from llm_toml_wizard.cli.briefing import DEMO_FACTS, BriefingSpec
from llm_toml_wizard.cli.queue import MainTurnQueue
from llm_toml_wizard.dialog import ACTION_CATALOG, ACTION_HANDLERS, DialogOrchestrator, pending_needs
from llm_toml_wizard.dialog.spec import DialogSpec
from llm_toml_wizard.dialog.state import DialogState
from llm_toml_wizard.toml_config.spec import (
    TOML_DEMO_AUTO,
    TOML_DEMO_DRAFT,
    TOML_DEMO_GHOST,
    TOML_DEMO_GOAL,
...
def _make_backend(args: Namespace):
    ...
    """
    if args.mock:
        from llm_toml_wizard.cli.mock_backend import ScriptedBackend
...
def cmd_catalog(args: Namespace) -> int:
    ...
    if spec.name == "toml":
        _print("domain actions (TomlGuideSpec weights; runtime is llm_toml_wizard.dialog):")
    else:
        _print("runtime actions (llm_toml_wizard.dialog, not toml_config):")
    _print(catalog)
    ...
    if _spec_name(args) == "toml":
        ...
        try:
            ...
            if (not args.mock) and args.release:
                from llm_lmstudio.facade import unload_model
                unload_model()
    facts: dict[str, str] = {}
    if args.demo:
